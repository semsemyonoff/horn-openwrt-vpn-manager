...
- `warnTopologyShift` is called from the pipeline (`subscription.go`, both the default and the `processSub` path) **after** include/exclude, not from `DecodePayload`. Filtering runs after decoding, so a subscription that excludes the new-scheme nodes stays single-node and warning from inside the decoder would be a false alarm repeated on every cron run. It is not called for inline `nodes` at all — those have no pre-multi-protocol tag to invalidate — and it takes the subscription id, since phase 2 decodes concurrently and an unattributed warning cannot be traced back.
