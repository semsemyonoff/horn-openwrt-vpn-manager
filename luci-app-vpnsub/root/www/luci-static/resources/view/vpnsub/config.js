...
var callSetProxy    = rpc.declare({ object: 'vpnsub', method: 'set_proxy',   params: ['proxy', 'selected'] });
var callTestDelays    = rpc.declare({ object: 'vpnsub', method: 'test_delays',    params: ['tags', 'url'] });
var callGetTemplate   = rpc.declare({ object: 'vpnsub', method: 'get_template' });
var callSetTemplate   = rpc.declare({ object: 'vpnsub', method: 'set_template',   params: ['template'] });
var callResetTemplate = rpc.declare({ object: 'vpnsub', method: 'reset_template' });
...
function makeLatencySpan(ms) {
	var cls = 'vpnsub-latency ' + (ms < 1000 ? 'vpnsub-latency-good' : 'vpnsub-latency-warn');
...
function highlightJson(raw, errLine) {
	...
	var highlighted = s.replace(
		...
		function(m, ph, key, colon, str, kw, num) {
			...
			if (key) return '<span class="hj-key">' + key + '</span>' + (colon || '');
...
function makeProxyWidget(subId, proxies, tagNames, intervalInput, toleranceInput) {
	...
	if (singleProxy) {
		...
		var name  = cleanNodeName((tagNames && tagNames[singleTag]) || singleTag);
	...
	var nodeTags  = allTags.filter(function(t) { return t !== autoTag; });
	var currentSelected = manualProxy.now || '';

	// Auto pane — current server + settings
	var autoNowName  = autoNow ? cleanNodeName((tagNames && tagNames[autoNow]) || autoNow) : null;
	...
	var autoPane = E('div', { 'class': 'vpnsub-proxy-auto-pane' }, [
		E('div', { 'class': 'vpnsub-proxy-current' }, [
			E('span', { 'class': 'vpnsub-proxy-current-label' }, _('Current') + ':\u00a0'),
			autoNow
				? E('span', { 'class': 'vpnsub-proxy-current-name' }, autoNowName || autoNow)
				: E('span', { 'class': 'vpnsub-proxy-current-none' }, '—'),
			autoNowDelay ? makeLatencySpan(autoNowDelay) : ''
		]),
		E('div', { 'class': 'vpnsub-proxy-auto-settings' }, [
			E('span', { 'class': 'vpnsub-proxy-setting-label' }, _('Interval') + ':\u00a0'),
			intervalInput,
			E('span', { 'class': 'vpnsub-proxy-setting-sep' }),
			E('span', { 'class': 'vpnsub-proxy-setting-label' }, _('Tolerance') + ':\u00a0'),
			toleranceInput,
	...
	function renderNodeList() {
		...
		sortedNodes.forEach(function(tag) {
			...
			var delayEl = delay !== null
				? makeLatencySpan(delay)
				: E('span', { 'class': 'vpnsub-no-latency' }, '\u2715');
			var row = E('div', {
				'class': 'vpnsub-proxy-node-row' + (isSelected ? ' vpnsub-proxy-selected' : '')
			}, [
				E('span', { 'class': 'vpnsub-proxy-node-name' }, name),
				delayEl
			]);
			...
			row.addEventListener('click', function() {
				callSetProxy(manualTag, tag).then(function() {
					...
					Array.prototype.forEach.call(
						...
						function(r) {
							r.classList.toggle('vpnsub-proxy-selected',
	...
	var testBtn = E('button', {
		...
		'click': function() {
			...
			var testUrl = (document.getElementById('vpnsub-test-url-setting') || {}).value || '';
			testUrl = testUrl.trim() || 'https://www.gstatic.com/generate_204';
			callTestDelays(nodeTags, testUrl).then(function(res) {
				if (res && res.delays) {
					nodeTags.forEach(function(tag) {
						var d = res.delays[tag];
						if (!proxies[tag]) proxies[tag] = { history: [] };
						if (!proxies[tag].history) proxies[tag].history = [];
						proxies[tag].history = [{ delay: (d && d > 0) ? d : 0 }];
					});
					sortedNodes = nodeTags.slice().sort(function(a, b) {
						var da = getLastDelay(proxies[a]);
						var db = getLastDelay(proxies[b]);
						if (da === null && db === null) return 0;
						if (da === null) return 1;
	...
	var toggleAuto = E('button', {
		'type': 'button',
		'class': 'vpnsub-mode-btn' + (!isManualMode ? ' vpnsub-mode-active' : ''),
		'click': function() {
...
function makeCollapsible(label, widget, description) {
	...
	}, [E('span', { 'class': 'vpnsub-collapse-toggle' }, [arrow, '\u00a0', label, '\u00a0', badge])]);
...
return view.extend({
	...
	load: function() {
		return Promise.all([ callGetConfig(), callGetLog(), callGetSbStatus(), callGetTemplate() ]);
	},
	...
	render: function(results) {
		...
		var globalSection = E('div', { 'class': 'cbi-section' }, [
			...
			formRow(_('Retries'), globalRetriesInput,
		...
		var settingsPane = E('div', { 'class': 'vpnsub-tab-pane' }, [
			...
			E('div', { 'class': 'cbi-page-actions' }, [saveBtn, '\u00a0\u00a0', settingsDirtyEl])
		]);

		// Mark dirty on any input/change inside the settings pane
		settingsPane.addEventListener('input',  function() { settingsDirtyEl.style.display = ''; });
		settingsPane.addEventListener('change', function() { settingsDirtyEl.style.display = ''; });
		...
		logPre.innerHTML = ansiToHtml(initialLog) || _('No log yet. Run the script to see output.');
		...
		var clearBtn = E('button', {
			'type': 'button',
			'class': 'cbi-button',
			'click': function() { logPre.innerHTML = ''; }
		}, _('Clear'));
		...
		var testPane = E('div', { 'class': 'vpnsub-tab-pane' }, [
			E('div', { 'class': 'cbi-section' }, [
				...
				E('div', { 'class': 'cbi-value-description' }, [
					_('Sends a request through tun0 and reports which outbound handled it.')
				]),
				testResultsEl
			])
		]);
		...
		var initialTemplate = (tmplData && tmplData.template) ? tmplData.template : '';
		...
		var templateStatusEl = E('div', { 'class': 'vpnsub-template-status', 'style': 'display:none' });
		...
		var dirtyEl = E('span', { 'class': 'vpnsub-template-dirty', 'style': 'display:none' },
			'\u25cf\u00a0' + _('Unsaved changes'));

		templateTextarea.addEventListener('input', function() {
			updateHighlight();
			dirtyEl.style.display = (templateTextarea.value !== originalTemplate) ? '' : 'none';
		});
		...
		var templateErrEl = E('div', { 'class': 'vpnsub-template-error', 'style': 'display:none' });

		var templateSaveBtn = E('button', {
			'type': 'button',
			'class': 'cbi-button cbi-button-save',
			'click': function() {
				var text = templateTextarea.value;
				...
				try {
					...
					templateErrEl.textContent = _('JSON error: ') + e.message;
					templateErrEl.style.display = '';
					return;
				}
				templateSaveBtn.disabled = true;
				callSetTemplate(text).then(function(res) {
					if (res && res.error) {
						templateErrEl.textContent = _('Error: ') + res.error;
						templateErrEl.style.display = '';
					} else {
					...
					templateErrEl.textContent = _('Error: ') + (err.message || String(err));
					templateErrEl.style.display = '';
				}).then(function() {
		...
		var templateResetBtn = E('button', {
			'type': 'button',
			'class': 'cbi-button',
			'click': function() {
				if (!window.confirm(_('Reset template to default?'))) return;
				templateResetBtn.disabled = true;
				callResetTemplate().then(function(res) {
					if (res && res.error) {
						templateErrEl.textContent = _('Error: ') + res.error;
						templateErrEl.style.display = '';
						...
						updateHighlight();
						templateErrEl.style.display = 'none';
						ui.addNotification(null, E('p', _('Template reset to default')), 'info');
					}
				}).catch(function(err) {
					templateErrEl.textContent = _('Error: ') + (err.message || String(err));
					templateErrEl.style.display = '';
		...
		var PLACEHOLDERS = [
			{ name: '__LOG_LEVEL__',       type: _('string'),
			  desc: _('Log verbosity level from global settings (e.g. warn, debug)') },
			{ name: '__VLESS_OUTBOUNDS__', type: _('array elements'),
			  desc: _('Individual VLESS proxy outbound objects, comma-separated') },
			{ name: '__GROUP_OUTBOUNDS__', type: _('array elements'),
			  desc: _('urltest (auto) and selector (manual) group outbound objects') },
			{ name: '__ROUTE_RULES__',     type: _('array elements'),
			  desc: _('Domain and IP routing rules generated from subscription settings') },
			{ name: '__DEFAULT_TAG__',     type: _('string'),
		...
		var templatePane = E('div', { 'class': 'vpnsub-tab-pane' }, [
			E('div', { 'class': 'cbi-section' }, [
				...
				E('div', { 'class': 'vpnsub-log-actions' }, [
					templateSaveBtn, '\u00a0', templateResetBtn, '\u00a0\u00a0', dirtyEl
				]),
				templateErrEl,
				templateWrap,
				templateStatusEl
		...
		return E('div', { 'class': 'cbi-map' }, [
			...
			makeTabs([
				...
				{ id: 'template', label: _('Template'),        content: templatePane }
			])
		]);
	},
	...
	_makeCard: function(sub, idx, proxies, tagNames) {
		...
		var removeBtn = E('button', {
			...
			'click': function() {
				...
				if (!window.confirm(_('Remove subscription') + ' "' + name + '"?')) return;
		...
		var proxyWidget = makeProxyWidget(subId, proxies, tagNames,
		...
		if (!hasUrltest) {
			children.push(formRow(_('Interval'), intervalInput,
				_('How often urltest checks latency (e.g. 5m, 1m)')));
			children.push(formRow(_('Tolerance'), toleranceInput,
				_('Switch to a faster node only if it beats current by at least this many ms')));
		}

		children.push(domainsRow);
		children.push(ipRow);
		children.push(formRow(_('Exclude filters'), excludeW.node,
	...
	_collectConfig: function() {
		...
		Array.prototype.forEach.call(
			...
			function(card) {
				...
				var id      = idEl ? idEl.value.trim() || sanitizeId(name) : sanitizeId(name);
		...
		var globalRetriesRaw = document.getElementById('vpnsub-retries').value.trim();
		var globalRetries = globalRetriesRaw !== '' ? parseInt(globalRetriesRaw, 10) : 3;
		return { log_level: level, test_url: testUrl, retries: globalRetries, subscriptions: subs };
	},
	...
	_validate: function() {
		...
		var defaults = subVals.filter(function(s) { return s.default === true; });
	...
	handleSave: function() {
		...
		return callSetConfig(cfg).then(function() {
			if (self._settingsDirtyEl) self._settingsDirtyEl.style.display = 'none';
	...
	_fetchSyslog: function(pre) {
		callGetSyslog(200).then(function(res) {
			...
			pre.innerHTML = ansiToHtml(text) || _('(no sing-box entries in syslog)');
		...
		callRunScript(dryRun, verbose).then(function() {
			self._pollTimer = setInterval(function() {
				callGetLog().then(function(res) {
					...
					logPre.innerHTML = ansiToHtml(logText) || _('(waiting for output…)');
